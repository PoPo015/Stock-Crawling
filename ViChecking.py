...
def setting_vi():
    ...
    for i in range(len(vi_result)):
        ...
        try:
            ...
            stock_info_array.append("00:00:00")
            stk_rel = '00:00:00'
            print(stk_nm+"종목 해제시각 null 대신 00:00:00 입력")

        result_dic  = {'stk_id':stk_id, 'stk_cd':stk_cd, 'stk_nm':stk_nm, 'stk_pri':stk_pri, 'stk_inc':stk_inc, 'stk_act':stk_act, 'stk_rel':stk_rel} # 딕셔너리 key:Value 값 입력
        convertJson_array.append(result_dic) #딕셔너리를 배열에 추가
        convertJson_dic = {'date_list': convertJson_array}  #다시 배열을 딕셔너리로 다중 key:value를 이용하기 위하여
        count += 11
        start += 1
    print("json 저장중")
